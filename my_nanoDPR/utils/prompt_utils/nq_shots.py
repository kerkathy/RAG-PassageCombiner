...
def get_nq_exemplars(lm_name, num_docs, num_exemplars):
    ...
    assert num_exemplars <= len(no_doc_exemplars), f"num_exemplars should be less than or equal to {len(no_doc_exemplars)}"
    exemplars = []
    if num_docs == 0:
        return no_doc_exemplars[:num_exemplars]
    for i in range(num_exemplars):
        doc = docs_supplement[i][:num_docs]
        ctxt_indicator = "Context" if "flan" in lm_name else "Document"
        exemplar = with_doc_exemplars[i].format(documents="\n".join(doc), ctxt_indicator=ctxt_indicator)
        exemplars.append(exemplar)
    return exemplars
# %%
# get_nq_exemplars("flan", 3, 5)
# %%
