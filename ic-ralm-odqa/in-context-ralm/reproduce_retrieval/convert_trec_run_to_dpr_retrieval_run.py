"""
This is a slightly modified version of the original script from Pyserini to convert TREC run to DPR retrieval result json.
The modification is that this script assumes there are only "text" in "contents", instead of "title" and "text" in "contents".
"""
...
def main(args):
    ...
    #         raise ValueError("Please provide a mapping file for hotpotid conversion")
    ...
    with open(args.input) as f_in:
        for line in tqdm(f_in.readlines()):
            ...
            if args.index == "msmarco-v2-passage":
                ctx = json.loads(searcher.doc(doc_id).raw())['passage']
            elif args.combine_title_text:
                passage = json.loads(searcher.doc(doc_id).raw())
                ctx = passage['title'] + "\n" + passage['text'] if 'title' in passage else passage['text']
            else:
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--hotpotid', action='store_true', default=False, help="Special hack for str id like hotpotqa")
    # parser.add_argument('--mapping-file', help='path to a mapping file containing str id to int id')
